...
class SecurityCenter5(BaseAPI):
    ...
    def __init__(self, host, port=443, ssl_verify=False, scheme='https',
        ...
        at: https://support.tenable.com/support-center/cerberus-support-center/
        includes/widgets/sc_api/index.html
        '''
    ...
    def login(self, user, passwd):
        '''Logs the user into SecurityCenter and stores the needed token and
        cookies.'''
    ...
    def analysis(self, *filters, **kwargs):
        ...
        The Analysis function also has a few functions that are slightly
        deviated from the API guides.  All of these options are optional,
        however can significantly change the how the API is being called.

        page         - Default "all"
                       The current page in the pagination sequence.

        page_size    - Default 1000
                       The page size (number of returned results)

        page_obj     - Default "return_results"
                       The class thats called after every API pull.

        page_kwargs  - Default {}
                       Any additional arguments that need to be passed
                       to the page_obj class when instantiated.

        type         - Default "vuln"
                       This is the type of data that will be returned.
        ...
        sourceType   - Default "cumulative"
                       This is how we specify individual scans, LCE archive
        ...
        if 'page_obj' not in kwargs:
            kwargs['page_obj'] = return_results
        if 'page_kwargs' not in kwargs:
            kwargs['page_kwargs'] = {}
        if 'type' not in kwargs:
            kwargs['type'] = 'vuln'
        if 'sourceType' not in kwargs:
            kwargs['sourceType'] = 'cumulative'
        if 'generator' in kwargs:
            kwargs['generator'] = return_generator

        # New we need to pull out the options from kwargs as we will be using
        # hwargs as the basis for the query that will be sent to SecurityCenter.
        ...
        if 'query' not in kwargs:
            kwargs['query'] = {
                'tool': kwargs['tool'],
                'type': kwargs['type'],
                'filters': [{'filterName': f[0], 'operator': f[1],
                             'value': f[2],
                             'type': kwargs['type']} for f in filters]
            }
            del kwargs['tool']
            if opts['page'] == 'all':
                ...
                kwargs['query']['startOffset'] = (opts['page'] *
                                                  opts['page_size'])
                kwargs['query']['endOffset'] = ((opts['page'] + 1) *
                                                opts['page_size'])
